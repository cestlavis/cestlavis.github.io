...
var circles = [[250, 350], [350, 450], [250, 150], [75, 250],   [75, 450], [225, 550], [400, 250]];
...
if(themelevel == 2){
	...
	fonts["material3"] = ["flowerfont", 18, 40];
	fonts["material4"] = ["radio", 24, 55];
...
function addIconPattern(iconname){
    ...
    if(iconpattern_ids.indexOf(iconpatternid) > -1) 
		...
		iconpattern.append("image") 
				...
				.attr("height", elsize)
				.attr("width", elsize)
				.attr("xlink:href", ('icons/'+material.icons+'/'+iconname+'.svg'));
				
		iconpattern_ids.push(iconpatternid);
		//console.log('added pattern: '+iconname);
		
		
		var iconpatternidH = iconname+'_patH';
		var iconpatternH = defs.append("pattern")
			.attr("id", iconpatternidH)
			//.attr("patternUnits", "userSpaceOnUse")
			.attr("height", 1)
			.attr("width", 1);
		
		iconpatternH.append("image") 
				.attr("id", iconname+'_imgH')
				.attr("height", elsize)
				.attr("width", elsize)
				.attr("xlink:href", ('icons/'+material.icons+'/'+iconname+'h.svg'));
