...
function stateObj(thematerial){
	...
	//console.log(thematerial.tickcount);
...
var deleteEx= function (){
	...
	//console.log("delete state: ", stateIndx);
	
	if(states.length > stateIndx){
		states.splice(stateIndx, 1);
		
		
		var butid = currentState;
        //console.log("here: ", butid,  document.getElementById(butid));
		removeElement( document.getElementById(butid) );	 
	}	
	rewriteURL();
	
	//now have to change id's of all exercise buttons to the right of the deleted one!!!
	for(var b=(stateIndx+1); b<(states.length+1); b++){
		var buttonid = 'ex'+b;
		var reducedid = 'ex'+(b-1);
		
		document.getElementById(buttonid).id = reducedid;
	}
	
	//now go back to previous recorded exercise if there is one, otherwise remain at zero
	if(stateIndx != 0)
    	stateIndx --;
   
    
	
	//console.log(states.length, stateIndx);
	
	if(states.length > 0){
		var prevstate = 'ex'+stateIndx;
		onClickButton(document.getElementById(prevstate).name, prevstate, false);
	}
	else{
	...
	if(states.length == 0){
		document.getElementById("replaceEx").style.visibility = "hidden";
		document.getElementById("deleteEx").style.visibility = "hidden";
	}
...
var replaceEx= function (){
	var stateIndx = parseInt(currentState.slice(2)); 
	//console.log("replace state: ", stateIndx);
	if(stateIndx < states.length){
...
var record = function () {
	...
	if(states.length > 0){
		document.getElementById("replaceEx").style.visibility = "visible";
		document.getElementById("deleteEx").style.visibility = "visible";
	}
...
function stateObjFromString(decoded){
	...
	this.tkcount = decoded['tkcount']; 
	this.det = (decoded['det'] == "true");
	this.text = decodeURIComponent(decoded['text']);
	
	this.tbl = decoded['tbl'];
	this.shfl = decoded['shfl'];
	this.add = decoded['add'];
...
function rewriteURL(){
	...
	if(states.length == 0){
    	history.pushState(thestate, null, null);
    	return;
    }
