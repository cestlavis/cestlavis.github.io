...
Group.prototype = {
    ...
    updateCount: function (numel){
    	...
    	var visuserel = Math.ceil(this.numuserelements / elcount);
    ...
    handleRemainder: function(pane){
    	...
    	var elcount = document.getElementById("elementcount").value;
    	var remainder = numels % elcount; 
    	
    	if(elcount == 1 || remainder == 0)
    		return;
    		
    	var topitem = Math.ceil(numels / elcount) - 1;
    	var height = elsize;
    ...
    populate:function(pop) {
		...
		if( material.isDetached == true || material.mode == 'cat' || material.mode == 'scale'){
			...
			var visel = Math.ceil(currcount / elcount);
			this.userdata = [];
