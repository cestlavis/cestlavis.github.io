...
function setTickMarks(){
	...
	var tickcnt = material.tickcount;
	
	var tickvals = [];
	
	for(var ind=0; ind<=elcount*10; ind++){
		if(tickcnt == 1){
			tickvals.push(ind);
		}	
		else{
...
function adjustCount(){
   ...
   if(elcount > 1){
	...
	for(var g=0; g<material.groups.length; g++){
		material.groups[g].data = [];
		
		var newcount = Math.ceil(material.groups[g].numelements / elcount);
		
		for(i=0; i<newcount; i++){
			material.groups[g].data.push(i);
		}
		
		
		if(material.isDetached){
			material.groups[g].userdata = [];
			var newusercount = Math.ceil(material.groups[g].numuserelements / elcount);
			
			for(i=0; i<newusercount; i++){
				material.groups[g].userdata.push(i);
			}
		}
	}
